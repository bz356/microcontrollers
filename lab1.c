...
static PT_THREAD (protothread_keypad(struct pt *pt))
{
    ...
    while(1) {
        ...
        // Scan period. This is the debounce interval: MAYBE_PRESSED is confirmed by
        // the NEXT scan, so a 30 ms period meant a press had to be held ~60 ms just
        // to register. 2 ms is long enough to outlast contact bounce.
        PT_YIELD_usec(2000) ;
    }
    // Indicate thread end
    PT_END(pt) ;
